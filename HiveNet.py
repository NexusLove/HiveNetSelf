...
stream_url = "www.youtube.com/c/NexusLoveYT"
tts_language = "en"
...
def startprint():
    ...
    print(f'''{Fore.RESET}
                       ...
                       {Fore.CYAN}HiveNet v{SELFBOT.__version__} | {Fore.GREEN}Logged in as: {HiveNet.user.name}#{HiveNet.user.discriminator} {Fore.CYAN}| ID: {Fore.GREEN}{HiveNet.user.id}   
                       {Fore.CYAN}Nitro Sniper | {Fore.GREEN}{nitro}
                       {Fore.CYAN}Cached Users: {Fore.GREEN}{len(HiveNet.users)}
                       {Fore.CYAN}Guilds: {Fore.GREEN}{len(HiveNet.guilds)}
                       {Fore.CYAN}Prefix: {Fore.GREEN}{HiveNet.command_prefix}
    ''' + Fore.RESET)
...
def Init():
    ...
    try:
        ...
        os.system(f'title (HiveNet Selfbot) - Version {SELFBOT.__version__}')
...
colorama.init()
HiveNet = discord.Client()
HiveNet = commands.Bot(description='HiveNet Selfbot', command_prefix=prefix, self_bot=True)

HiveNet.antiraid = False
HiveNet.msgsniper = True
HiveNet.slotbot_sniper = True
HiveNet.giveaway_sniper = True
HiveNet.mee6 = False
HiveNet.mee6_channel = None
HiveNet.yui_kiss_user = None
HiveNet.yui_kiss_channel = None
HiveNet.yui_hug_user = None
HiveNet.yui_hug_channel = None
HiveNet.sniped_message_dict = {}
HiveNet.sniped_edited_message_dict = {}
HiveNet.whitelisted_users = {}
HiveNet.copycat = None
HiveNet.remove_command('help')
...
async def on_message_edit(before, after):
    await HiveNet.process_commands(after)


@HiveNet.event
async def on_message(message):
    if HiveNet.copycat is not None and HiveNet.copycat.id == message.author.id:
    ...
    if f'Congratulations <@{HiveNet.user.id}>' in message.content:
        if HiveNet.giveaway_sniper:
            if message.author.id == 294882584201003009:
                print(""
    ...
    await HiveNet.process_commands(message)


@HiveNet.event
async def on_member_ban(guild: discord.Guild, user: discord.user):
    if HiveNet.antiraid is True:
        try:
            async for i in guild.audit_logs(limit=1, action=discord.AuditLogAction.ban):
                if guild.id in HiveNet.whitelisted_users.keys() and i.user.id in HiveNet.whitelisted_users[
                    guild.id].keys() and i.user.id is not HiveNet.user.id:
                    print("not banned - " + i.user.name)
                else:
                    print("banned - " + i.user.name)
                    await guild.ban(i.user, reason="HiveNet Anti-Nuke")
...
async def on_member_join(member):
    if HiveNet.antiraid is True and member.bot:
        try:
            guild = member.guild
            async for i in guild.audit_logs(limit=1, action=discord.AuditLogAction.bot_add):
                if member.guild.id in HiveNet.whitelisted_users.keys() and i.user.id in HiveNet.whitelisted_users[
                    member.guild.id].keys():
                    return
                else:
                    await guild.ban(member, reason="HiveNet Anti-Nuke")
                    await guild.ban(i.user, reason="HiveNet Anti-Nuke")
        except Exception as e:
            print(e)
...
@HiveNet.command(aliases=["queue"])
async def play(ctx, *, query):
    await ctx.message.delete()
    voice = get(HiveNet.voice_clients, guild=ctx.guild)
...
async def msgsniper(ctx, msgsniperlol=None):
    ...
    if str(msgsniperlol).lower() == 'true' or str(msgsniperlol).lower() == 'on':
        HiveNet.msgsniper = True
        await ctx.send('HiveNet Message-Sniper is now **enabled**')
    elif str(msgsniperlol).lower() == 'false' or str(msgsniperlol).lower() == 'off':
        HiveNet.msgsniper = False
        await ctx.send('HiveNet Message-Sniper is now **disabled**')


@HiveNet.command(aliases=['ar', 'antiraid'])
async def antinuke(ctx, antiraidparameter=None):
    await ctx.message.delete()
    HiveNet.antiraid = False
    if str(antiraidparameter).lower() == 'true' or str(antiraidparameter).lower() == 'on':
        HiveNet.antiraid = True
...
@HiveNet.command(aliases=['wl'])
async def whitelist(ctx, user: discord.Member = None):
    await ctx.message.delete()
    if user is None:
        await ctx.send("Please specify a user to whitelist")
    else:
        if ctx.guild.id not in HiveNet.whitelisted_users.keys():
            HiveNet.whitelisted_users[ctx.guild.id] = {}
        if user.id in HiveNet.whitelisted_users[ctx.guild.id]:
            await ctx.send('That user is already whitelisted')
        else:
            HiveNet.whitelisted_users[ctx.guild.id][user.id] = 0
            await ctx.send("Whitelisted **" + user.name.replace("*", "\*").replace("`", "\`").replace("_",
                                                                                                      "\_") + "#" + user.discriminator + "**")
    # else:
    #     user = HiveNet.get_user(id)
    #     if user is None:
...
@HiveNet.command(aliases=['wld'])
async def whitelisted(ctx, g=None):
    await ctx.message.delete()
    if g == '-g' or g == '-global':
        whitelist = '`All Whitelisted Users:`\n'
        for key in HiveNet.whitelisted_users:
            for key2 in HiveNet.whitelisted_users[key]:
                user = HiveNet.get_user(key2)
                whitelist += '**+ ' + user.name.replace('*', "\*").replace('`', "\`").replace('_',
                                                                                              "\_") + "#" + user.discriminator + "** - " + HiveNet.get_guild(
        ...
        for key in HiveNet.whitelisted_users:
            if key == ctx.guild.id:
                for key2 in HiveNet.whitelisted_users[ctx.guild.id]:
                    user = HiveNet.get_user(key2)
                    whitelist += '**+ ' + user.name.replace('*', "\*").replace('`', "\`").replace('_',
...
async def unwhitelist(ctx, user: discord.Member = None):
    if user is None:
        ...
        if ctx.guild.id not in HiveNet.whitelisted_users.keys():
            ...
            return
        if user.id in HiveNet.whitelisted_users[ctx.guild.id]:
            HiveNet.whitelisted_users[ctx.guild.id].pop(user.id, 0)
            user2 = HiveNet.get_user(user.id)
            await ctx.send(
                'Successfully unwhitelisted **' + user2.name.replace('*', "\*").replace('`', "\`").replace('_',
                                                                                                           "\_") + '#' + user2.discriminator + '**')


@HiveNet.command(aliases=['clearwl', 'clearwld'])
async def clearwhitelist(ctx):
    await ctx.message.delete()
    HiveNet.whitelisted_users.clear()
...
async def yuikiss(ctx, user: discord.User = None):
    ...
    if isinstance(ctx.message.channel, discord.DMChannel) or isinstance(ctx.message.channel, discord.GroupChannel):
        ...
        if user is None:
            ...
            return
        HiveNet.yui_kiss_user = user.id
        HiveNet.yui_kiss_channel = ctx.channel.id
        if HiveNet.yui_kiss_user is None or HiveNet.yui_kiss_channel is None:
            await ctx.send('An impossible error occured, try again later or contact swag')
            return
        while HiveNet.yui_kiss_user is not None and HiveNet.yui_kiss_channel is not None:
            await HiveNet.get_channel(HiveNet.yui_kiss_channel).send('yui kiss ' + str(HiveNet.yui_kiss_user), delete_after=0.1)
...
async def yuihug(ctx, user: discord.User = None):
    ...
    if isinstance(ctx.message.channel, discord.DMChannel) or isinstance(ctx.message.channel, discord.GroupChannel):
        ...
        if user is None:
            ...
            return
        HiveNet.yui_hug_user = user.id
        HiveNet.yui_hug_channel = ctx.channel.id
        if HiveNet.yui_hug_user is None or HiveNet.yui_hug_channel is None:
            await ctx.send('An impossible error occured, try again later or contact swag')
            return
        while HiveNet.yui_hug_user is not None and HiveNet.yui_hug_channel is not None:
            await HiveNet.get_channel(HiveNet.yui_hug_channel).send('yui hug ' + str(HiveNet.yui_hug_user), delete_after=0.1)
            await asyncio.sleep(60)
...
@HiveNet.command(aliases=["automee6"])
async def mee6(ctx, param=None):
    ...
    while HiveNet.mee6 is True:
        ...
        await HiveNet.get_channel(HiveNet.mee6_channel).send(random.choice(sentences), delete_after=0.1)
        await asyncio.sleep(60)


@HiveNet.command(aliases=['slotsniper', "slotbotsniper"])
async def slotbot(ctx, param=None):
    await ctx.message.delete()
    HiveNet.slotbot_sniper = False
    if str(param).lower() == 'true' or str(param).lower() == 'on':
        HiveNet.slotbot_sniper = True
    elif str(param).lower() == 'false' or str(param).lower() == 'off':
        HiveNet.slotbot_sniper = False
...
@HiveNet.command(aliases=['giveawaysniper'])
async def giveaway(ctx, param=None):
    await ctx.message.delete()
    HiveNet.giveaway_sniper = False
    if str(param).lower() == 'true' or str(param).lower() == 'on':
        HiveNet.giveaway_sniper = True
    elif str(param).lower() == 'false' or str(param).lower() == 'off':
        HiveNet.giveaway_sniper = False
...
async def on_message_delete(message):
    if message.author.id == HiveNet.user.id:
        return
    if HiveNet.msgsniper:
        if isinstance(message.channel, discord.DMChannel) or isinstance(message.channel, discord.GroupChannel):
            ...
            if len(attachments) == 0:
                ...
                message_content = "`" + str(
                    ...
                    message.content) + "\n\n**Attachments:**\n" + links
                await message.channel.send(message_content)
    if len(HiveNet.sniped_message_dict) > 1000:
        HiveNet.sniped_message_dict.clear()
    attachments = message.attachments
    if len(attachments) == 0:
        channel_id = message.channel.id
        message_content = "`" + str(discord.utils.escape_markdown(str(message.author))) + "`: " + str(
            message.content).replace("@everyone", "@\u200beveryone").replace("@here", "@\u200bhere")
        HiveNet.sniped_message_dict.update({channel_id: message_content})
    else:
        links = ""
        for attachment in attachments:
            links += attachment.proxy_url + "\n"
...
@HiveNet.event
async def on_message_edit(before, after):
    if before.author.id == HiveNet.user.id:
        return
    if HiveNet.msgsniper:
        ...
        if isinstance(before.channel, discord.DMChannel) or isinstance(before.channel, discord.GroupChannel):
            ...
            if len(attachments) == 0:
                ...
                message_content = "`" + str(
                    ...
                    before.content) + "\n\n**Attachments:**\n" + links
                await before.channel.send(message_content)
    if len(HiveNet.sniped_edited_message_dict) > 1000:
        HiveNet.sniped_edited_message_dict.clear()
    attachments = before.attachments
    if len(attachments) == 0:
        ...
        message_content = "`" + str(discord.utils.escape_markdown(str(before.author))) + "`: \n**BEFORE**\n" + str(
            before.content).replace("@everyone", "@\u200beveryone").replace("@here",
                                                                            "@\u200bhere") + "\n**AFTER**\n" + str(
            after.content).replace("@everyone", "@\u200beveryone").replace("@here", "@\u200bhere")
        HiveNet.sniped_edited_message_dict.update({channel_id: message_content})
    else:
        links = ""
        for attachment in attachments:
            links += attachment.proxy_url + "\n"
...
async def snipe(ctx):
    ...
    if currentChannel in HiveNet.sniped_message_dict:
        await ctx.send(HiveNet.sniped_message_dict[currentChannel])
    else:
        await ctx.send("No message to snipe!")


@HiveNet.command(aliases=["esnipe"])
async def editsnipe(ctx):
    await ctx.message.delete()
    currentChannel = ctx.channel.id
    if currentChannel in HiveNet.sniped_edited_message_dict:
        await ctx.send(HiveNet.sniped_edited_message_dict[currentChannel])
    else:
        await ctx.send("No message to snipe!")
...
async def help(ctx, category=None):
    ...
    if category is None:
        ...
        embed.set_author(name="HIVENET 𝙎𝙀𝙇𝙁𝘽𝙊𝙏 | 𝙋𝙍𝙀𝙁𝙄𝙓: " + str(HiveNet.command_prefix),
                         icon_url=HiveNet.user.avatar_url)
        embed.set_thumbnail(url=HiveNet.user.avatar_url)
...
@HiveNet.command(aliases=["giphy", "tenor", "searchgif"])
...
@HiveNet.command(aliases=["img", "searchimg", "searchimage", "imagesearch", "imgsearch"])
    ...
    if str(image_tags[2]['src']).find("https://trkn.us/pixel/imp/c="):
        ...
        try:
            ...
            with io.BytesIO(image) as file:
                await ctx.send(f"Search result for: **{args}**", file=discord.File(file, f"hivenet_anal.png"))
        except:
...
@HiveNet.command(aliases=["addemoji", "stealemote", "addemote"])
...
@HiveNet.command(aliases=["stopcopycatuser", "stopcopyuser", "stopcopy"])
async def stopcopycat(ctx):
    await ctx.message.delete()
    if HiveNet.user is None:
        await ctx.send("You weren't copying anyone to begin with")
        return
    await ctx.send("Stopped copying " + str(HiveNet.copycat))
    HiveNet.copycat = None


@HiveNet.command(aliases=["copycatuser", "copyuser"])
async def copycat(ctx, user: discord.User):
    await ctx.message.delete()
    HiveNet.copycat = user
    await ctx.send("Now copying " + str(HiveNet.copycat))


@HiveNet.command(aliases=["9/11", "911", "terrorist"])
...
@HiveNet.command(aliases=["jerkoff", "ejaculate", "orgasm"])
async def cum(ctx):
    await ctx.message.delete()
...
@HiveNet.command(aliases=["spamchangegcname", "changegcname"])
async def spamgcname(ctx):
    await ctx.message.delete()
    if isinstance(ctx.message.channel, discord.GroupChannel):
        watermark = "HiveNet LOL"
        name = ""
...
@HiveNet.command(aliases=['geolocate', 'iptogeo', 'iptolocation', 'ip2geo', 'ip'])
            ...
            try:
                async with aiohttp.ClientSession() as session:
                    async with session.get(str(res['message'])) as resp:
                        image = await resp.read()
                with io.BytesIO(image) as file:
                    await ctx.send(file=discord.File(file, f"hivenet_tweet.png"))
            except:
                await ctx.send(res['message'])


@HiveNet.command(aliases=["distort"])
        ...
        try:
            async with aiohttp.ClientSession() as session:
                async with session.get(str(res['message'])) as resp:
                    image = await resp.read()
            with io.BytesIO(image) as file:
                await ctx.send(file=discord.File(file, f"hivenet_magik.png"))
        except:
            await ctx.send(res['message'])
    else:
...
@HiveNet.command(aliases=['markasread', 'ack'])
async def read(ctx):
    await ctx.message.delete()
    for guild in HiveNet.guilds:
        await guild.ack()
...
@HiveNet.command(aliases=["deepfry"])
        ...
        try:
            async with aiohttp.ClientSession() as session:
                async with session.get(str(res['message'])) as resp:
                    image = await resp.read()
            with io.BytesIO(image) as file:
                await ctx.send(file=discord.File(file, f"hivenet_fry.png"))
        except:
            await ctx.send(res['message'])
    else:
        avatar = str(user.avatar_url_as(format="png"))
        ...
        try:
            async with aiohttp.ClientSession() as session:
                async with session.get(endpoint) as resp:
                    image = await resp.read()
            with io.BytesIO(image) as file:
                await ctx.send(file=discord.File(file, f"hivenet_blur.png"))
        except:
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(endpoint) as resp:
                image = await resp.read()
        with io.BytesIO(image) as file:
            await ctx.send(file=discord.File(file, f"hivenet_supreme.png"))
    except:
        await ctx.send(endpoint)
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(endpoint) as resp:
                image = await resp.read()
        with io.BytesIO(image) as file:
            await ctx.send(file=discord.File(file, f"hivenet_dark_supreme.png"))
    except:
        await ctx.send(endpoint)


@HiveNet.command(aliases=["facts"])
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(endpoint) as resp:
                image = await resp.read()
        with io.BytesIO(image) as file:
            await ctx.send(file=discord.File(file, f"hivenet_facts.png"))
    except:
        await ctx.send(endpoint)


@HiveNet.command(aliases=["blurp"])
        ...
        try:
            async with aiohttp.ClientSession() as session:
                async with session.get(str(res['message'])) as resp:
                    image = await resp.read()
            with io.BytesIO(image) as file:
                await ctx.send(file=discord.File(file, f"hivenet_blurpify.png"))
        except:
            await ctx.send(res['message'])
    else:
        avatar = str(user.avatar_url_as(format="png"))
        ...
        try:
            async with aiohttp.ClientSession() as session:
                async with session.get(endpoint) as resp:
                    image = await resp.read()
            with io.BytesIO(image) as file:
                await ctx.send(file=discord.File(file, f"hivenet_invert.png"))
        except:
            await ctx.send(endpoint)
    else:
        avatar = str(user.avatar_url_as(format="png"))
...
@HiveNet.command(aliases=["pornhublogo", "phlogo"])
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(endpoint) as resp:
                image = await resp.read()
        with io.BytesIO(image) as file:
            await ctx.send(file=discord.File(file, f"hivenet_pornhub_logo.png"))
    except:
        await ctx.send(endpoint)


@HiveNet.command(aliases=["pornhubcomment", 'phc'])
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(res["message"]) as resp:
                image = await resp.read()
        with io.BytesIO(image) as file:
            await ctx.send(file=discord.File(file, f"hivenet_pornhub_comment.png"))
    except:
        await ctx.send(res["message"])
...
@HiveNet.command(aliases=["reversesearch", "anticatfish", "catfish"])
...
@HiveNet.command(aliases=['pfp', 'avatar'])
...
@HiveNet.command(aliases=["del", "quickdel"])
...
@HiveNet.command(name='1337speak', aliases=['leetspeak'])
...
async def ghost(ctx):
    ...
    if config.get('password') == 'password-here':
        ...
        with open('Images/Avatars/Transparent.png', 'rb') as f:
            try:
                await HiveNet.user.edit(password=password, username="ٴٴٴٴ", avatar=f.read())
            except discord.HTTPException as e:
                print(f"{Fore.RED}[ERROR]: {Fore.YELLOW}{e}" + Fore.RESET)


@HiveNet.command(aliases=['pfpget', 'stealpfp'])
    ...
    if config.get('password') == 'password-here':
        ...
        try:
            ...
            with open('Images/Avatars/Stolen/Stolen.png', 'rb') as f:
                await HiveNet.user.edit(password=password, avatar=f.read())
        except discord.HTTPException as e:
            print(f"{Fore.RED}[ERROR]: {Fore.YELLOW}{e}" + Fore.RESET)


@HiveNet.command(name='set-pfp', aliases=['setpfp', 'pfpset,"changepfp'])
...
@HiveNet.command(aliases=['wouldyourather', 'would-you-rather', 'wyrq'])
...
@HiveNet.command(aliases=['dong', 'penis'])
...
@HiveNet.command(aliases=['tokenfucker', 'disable', 'crash'])
...
@HiveNet.command(aliases=['fakeconnection', 'spoofconnection', 'spoofcon', "fakecon"])
async def fakenet(ctx, _type=None, *, name=None):
    ...
    if r.status_code == 200:
        ...
        await ctx.send(
            '**[ERROR]** `HiveNet Fake-Connection doesn\'t work anymore because Discord patched connection-spoofing`')


@HiveNet.command(aliases=['tokinfo', 'tdox'])
...
@HiveNet.command(aliases=["copyguild", "copyserver"])
async def copy(ctx):  # b'\xfc'
    await ctx.message.delete()
    await HiveNet.create_guild(f'backup-{ctx.guild.name}')
    await asyncio.sleep(4)
    for g in HiveNet.guilds:
...
@HiveNet.command(aliases=["rekt", "nuke"])
async def destroy(ctx):
    ...
    try:
        ...
        pass
    for _i in range(250):
        await ctx.guild.create_text_channel(name="hivenet")
    for _i in range(250):
        await ctx.guild.create_role(name="hivenet", color=RandomColor())


@HiveNet.command(aliases=["banwave", "banall", "etb"])
async def massban(ctx):
    await ctx.message.delete()
    users = list(ctx.guild.members)
    for user in users:
        try:
            await user.ban(reason="hivenet")
        except:
            pass
...
@HiveNet.command(aliases=["kickall", "kickwave"])
async def masskick(ctx):
    await ctx.message.delete()
    users = list(ctx.guild.members)
    for user in users:
        try:
            await user.kick(reason="hivenet")
        except:
            pass


@HiveNet.command(aliases=["spamroles"])
...
@HiveNet.command(aliases=["masschannels", "masschannel", "ctc"])
...
@HiveNet.command(aliases=["delchannel"])
...
@HiveNet.command(aliases=["deleteroles"])
...
@HiveNet.command(aliases=["purgebans", "unbanall"])
...
async def dm(ctx, user: discord.Member, *, message):
    await ctx.message.delete()
    user = HiveNet.get_user(user.id)
    if ctx.author.id == HiveNet.user.id:
        return
    else:
        try:
...
@HiveNet.command(name='get-color', aliases=['color', 'colour', 'sc', "hexcolor", "rgb"])
...
@HiveNet.command(aliases=['rainbowrole'])
...
@HiveNet.command(aliases=["guildinfo"])
...
@HiveNet.command(aliases=['slots', 'bet', "slotmachine"])
async def slot(ctx):
    await ctx.message.delete()
    emojis = "🍎🍊🍐🍋🍉🍇🍓🍒"
...
@HiveNet.command(aliases=['guildpfp', 'serverpfp', 'servericon'])
...
@HiveNet.command(aliases=['serverbanner'])
...
@HiveNet.command(name='first-message', aliases=['firstmsg', 'fm', 'firstmessage'])
...
@HiveNet.command(aliases=["renameserver", "nameserver"])
...
async def prefix(ctx, prefix):
    await ctx.message.delete()
    HiveNet.command_prefix = str(prefix)
...
@HiveNet.command(aliases=['bitcoin'])
async def btc(ctx):
    await ctx.message.delete()
...
@HiveNet.command(pass_context=True, aliases=["cyclename", "autoname", "autonick", "cycle"])
...
@HiveNet.command(aliases=["stopcyclename", "cyclestop", "stopautoname", "stopautonick", "stopcycle"])
...
async def acceptfriends(ctx):
    await ctx.message.delete()
    for relationship in HiveNet.user.relationships:
...
async def clearblocked(ctx):
    await ctx.message.delete()
    print(HiveNet.user.relationships)
    for relationship in HiveNet.user.relationships:
...
@HiveNet.command(aliases=["changeregions", "changeregion", "regionschange"])
...
@HiveNet.command(aliases=["gcleave"])
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(link) as resp:
                image = await resp.read()
        with io.BytesIO(image) as file:
            await ctx.send(file=discord.File(file, f"hivenet_dog.png"))
    except:
        await ctx.send(link)
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(link) as resp:
                image = await resp.read()
        with io.BytesIO(image) as file:
            await ctx.send(file=discord.File(file, f"hivenet_cat.png"))
    except:
        await ctx.send(link)
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(link) as resp:
                image = await resp.read()
        with io.BytesIO(image) as file:
            await ctx.send(file=discord.File(file, f"hivenet_sadcat.png"))
    except:
        await ctx.send(link)
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(link) as resp:
                image = await resp.read()
        with io.BytesIO(image) as file:
            await ctx.send(file=discord.File(file, f"hivenet_bird.png"))
    except:
        await ctx.send(link)
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(link) as resp:
                image = await resp.read()
        with io.BytesIO(image) as file:
            await ctx.send(file=discord.File(file, f"hivenet_fox.png"))
    except:
        await ctx.send(link)
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(res['url']) as resp:
                image = await resp.read()
        with io.BytesIO(image) as file:
            await ctx.send(file=discord.File(file, f"hivenet_anal.gif"))
    except:
        em = discord.Embed()
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(res['url']) as resp:
                image = await resp.read()
        with io.BytesIO(image) as file:
            await ctx.send(file=discord.File(file, f"hivenet_erofeet.png"))
    except:
        em = discord.Embed()
        em.set_image(url=res['url'])
        await ctx.send(embed=em)
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(res['url']) as resp:
                image = await resp.read()
        with io.BytesIO(image) as file:
            await ctx.send(file=discord.File(file, f"hivenet_feet.gif"))
    except:
        em = discord.Embed()
        em.set_image(url=res['url'])
        await ctx.send(embed=em)
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(res['url']) as resp:
                image = await resp.read()
        with io.BytesIO(image) as file:
            await ctx.send(file=discord.File(file, f"hivenet_hentai.gif"))
    except:
        em = discord.Embed()
        em.set_image(url=res['url'])
        await ctx.send(embed=em)
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(res['url']) as resp:
                image = await resp.read()
        with io.BytesIO(image) as file:
            await ctx.send(file=discord.File(file, f"hivenet_boobs.gif"))
    except:
        em = discord.Embed()
        em.set_image(url=res['url'])
        await ctx.send(embed=em)
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(res['url']) as resp:
                image = await resp.read()
        with io.BytesIO(image) as file:
            await ctx.send(file=discord.File(file, f"hivenet_tits.gif"))
    except:
        em = discord.Embed()
        em.set_image(url=res['url'])
        await ctx.send(embed=em)
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(res['url']) as resp:
                image = await resp.read()
        with io.BytesIO(image) as file:
            await ctx.send(file=discord.File(file, f"hivenet_neko.gif"))
    except:
        em = discord.Embed()
        em.set_image(url=res['url'])
        await ctx.send(embed=em)
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(res['url']) as resp:
                image = await resp.read()
        with io.BytesIO(image) as file:
            await ctx.send(file=discord.File(file, f"hivenet_lesbian.gif"))
    except:
        em = discord.Embed()
        em.set_image(url=res['url'])
        await ctx.send(embed=em)
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(res['url']) as resp:
                image = await resp.read()
        with io.BytesIO(image) as file:
            await ctx.send(file=discord.File(file, f"hivenet_cumslut.gif"))
    except:
        em = discord.Embed()
        em.set_image(url=res['url'])
        await ctx.send(embed=em)
...
@HiveNet.command(aliases=["vagina"])
async def pussy(ctx):
    await ctx.message.delete()
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(res['url']) as resp:
                image = await resp.read()
        with io.BytesIO(image) as file:
            await ctx.send(file=discord.File(file, f"hivenet_pussy.gif"))
    except:
        em = discord.Embed()
        em.set_image(url=res['url'])
        await ctx.send(embed=em)
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(res['url']) as resp:
                image = await resp.read()
        with io.BytesIO(image) as file:
            await ctx.send(file=discord.File(file, f"hivenet_waifu.gif"))
    except:
        em = discord.Embed()
        em.set_image(url=res['url'])
        await ctx.send(embed=em)
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(res['url']) as resp:
                image = await resp.read()
        with io.BytesIO(image) as file:
            await ctx.send(user.mention, file=discord.File(file, f"hivenet_feed.gif"))
    except:
        em = discord.Embed(description=user.mention)
        em.set_image(url=res['url'])
        await ctx.send(embed=em)
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(res['url']) as resp:
                image = await resp.read()
        with io.BytesIO(image) as file:
            await ctx.send(user.mention, file=discord.File(file, f"hivenet_tickle.gif"))
    except:
        em = discord.Embed(description=user.mention)
        em.set_image(url=res['url'])
        await ctx.send(embed=em)
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(res['url']) as resp:
                image = await resp.read()
        with io.BytesIO(image) as file:
            await ctx.send(user.mention, file=discord.File(file, f"hivenet_slap.gif"))
    except:
        em = discord.Embed(description=user.mention)
        em.set_image(url=res['url'])
        await ctx.send(embed=em)
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(res['url']) as resp:
                image = await resp.read()
        with io.BytesIO(image) as file:
            await ctx.send(user.mention, file=discord.File(file, f"hivenet_hug.gif"))
    except:
        em = discord.Embed(description=user.mention)
        em.set_image(url=res['url'])
        await ctx.send(embed=em)
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(res['url']) as resp:
                image = await resp.read()
        with io.BytesIO(image) as file:
            await ctx.send(user.mention, file=discord.File(file, f"hivenet_cuddle.gif"))
    except:
        em = discord.Embed(description=user.mention)
        em.set_image(url=res['url'])
        await ctx.send(embed=em)
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(res['url']) as resp:
                image = await resp.read()
        with io.BytesIO(image) as file:
            await ctx.send(user.mention, file=discord.File(file, f"hivenet_smug.gif"))
    except:
        em = discord.Embed(description=user.mention)
        em.set_image(url=res['url'])
        await ctx.send(embed=em)
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(res['url']) as resp:
                image = await resp.read()
        with io.BytesIO(image) as file:
            await ctx.send(user.mention, file=discord.File(file, f"hivenet_pat.gif"))
    except:
        em = discord.Embed(description=user.mention)
        em.set_image(url=res['url'])
        await ctx.send(embed=em)
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(res['url']) as resp:
                image = await resp.read()
        with io.BytesIO(image) as file:
            await ctx.send(user.mention, file=discord.File(file, f"hivenet_kiss.gif"))
    except:
        em = discord.Embed(description=user.mention)
        em.set_image(url=res['url'])
        await ctx.send(embed=em)
...
async def purge(ctx, amount: int):
    await ctx.message.delete()
    async for message in ctx.message.channel.history(limit=amount).filter(lambda m: m.author == HiveNet.user).map(
            lambda m: m):
        try:
            await message.delete()
        except:
...
@HiveNet.command(name='group-leaver',
                aliase=['leaveallgroups', 'leavegroup', 'leavegroups', "groupleave", "groupleaver"])
async def _group_leaver(ctx):
    await ctx.message.delete()
    for channel in HiveNet.private_channels:
        if isinstance(channel, discord.GroupChannel):
            await channel.leave()


@HiveNet.command(aliases=["streaming"])
async def stream(ctx, *, message):
    ...
    await HiveNet.change_presence(activity=stream)
...
@HiveNet.command(aliases=["listen"])
...
@HiveNet.command(aliases=["fliptable"])
...
@HiveNet.command(name='rolecolor')
...
@HiveNet.command(aliases=["logout"])
async def shutdown(ctx):
    await ctx.message.delete()
    await HiveNet.logout()


@HiveNet.command(aliases=["nitrogen"])
async def nitro(ctx):
    await ctx.message.delete()
